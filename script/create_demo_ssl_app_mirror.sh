export AMQ_URL=messaging-1.apps.cluster-80e6.80e6.sandbox118.opentlc.com
...
export REMOTE_CONNECTOR_SECRET_NAME=server-secret
...
oc -n ${AMQ_ONLINE_INFRA_PROJ} delete secret ${REMOTE_CONNECTOR_SECRET_NAME}
oc -n ${AMQ_ONLINE_INFRA_PROJ} create secret tls ${EXPOSE_ENDPOINT_SECRET_NAME} --cert=${AMQ_KEYSTORE}_cert.crt --key=$AMQ_KEYSTORE.key


sed -i'' -e "s/routeHost:.*/routeHost: $AMQ_URL/g" ../demo-ssl-objects.yaml


oc create -f ../demo-ssl-objects.yaml
