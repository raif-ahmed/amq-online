= Understanding AMQ Online Components
...
* AuthenticationService - Describes an authentication service instance used to authenticate massaging clients. This is simply your way to configure keycloak server itself.

* AddressSpacePlan - Describes the messaging resources available for address spaces (will come later) using this plan, such as the available address plans and the amount of router and broker resources that can be used.
...
** StandardInfraConfig - This will allow you to host both AMQ Brokers, and AMQ Interconnect routes

** BrokeredInfraConfig - This will allow you to host both AMQ Brokers only.
...
.. Define AddressPlan
. Each Tenant will create needed objects in his
.. Define AddressSpace
.. Define Address
.. Define MessageUser
